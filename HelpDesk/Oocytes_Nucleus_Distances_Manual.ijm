...
for (i = 0; i < inputFiles.length; i++) {
    if (endsWith(inputFiles[i], ".nd")) {
		...
		// Manual thresholding, asking the user to specify the lower threshold
		run("Threshold...");
		setAutoThreshold("MaxEntropy dark stack");
		getThreshold(lower, upper);
		Dialog.createNonBlocking("488 channel");
		Dialog.addNumber("Lower threshold", lower);
		Dialog.addMessage("Please make sure 'Stack histogram' is ON and 'Don't reset range' is OFF");
		Dialog.show();	
		th_488 = Dialog.getNumber();
		resetThreshold;
		close("Threshold");
		setThreshold(th_488, 65535);
		run("Convert to Mask","stack black");
		...
		setAutoThreshold("MaxEntropy dark stack");
		getThreshold(lower, upper);
		Dialog.createNonBlocking("561 channel");
		Dialog.addNumber("Lower threshold", lower);
		Dialog.addMessage("Please make sure 'Stack histogram' is ON and 'Don't reset range' is OFF");
		Dialog.show();	
		th_561 = Dialog.getNumber();
		resetThreshold;
		close("Threshold");
		setThreshold(th_561, 65535);
		run("Convert to Mask","stack black");
